...
//	#include "opencv2/ocl/ocl.hpp" TESTING OPENCV OCL MODULE TO FIX RESIZE ISSUE
...
//Resize(outputdwidth,outputheight,input_imagecolor,input_imagedepth,output_imagecolor,output_imagedepth)
  ...
  void imageViewer()
  {
    ...
    for(; running && ros::ok();)
    {
      if(updateImage)
      {
		...
		//cv::resize(color,color,cv::Size(640,480),0.0,0.0,cv::INTER_CUBIC);
		//cv::resize(depthDisp,depthDisp,cv::Size(640,480),0.0,0.0,cv::INTER_CUBIC);
  ...
  void dispDepth(const cv::Mat &in, cv::Mat &out, const float maxValue)
  {
	...
	// TESTCODE FOR RESIZE USING OPENCV OCL MODULE	
	// START
	//cv::Mat tmp = cv::Mat(in.rows, in.cols, CV_8U);
	//ocl::resize(in, tmp, sizetest,0,0,0,INTER_NEAREST);	
	//END    

	//cv::resize(in, tmp, cv::Size(in.cols * 1,in.rows * 1), 0, 0, CV_INTER_AREA);	
	//cv::resize(in, tmp, cv::Size(640, 480));
 
    cv::Mat tmp = cv::Mat(in.rows, in.cols, CV_8U);
 	//cv::Mat out = cv::Mat(in.rows, in.cols, CV_8U);
    ...
    cv::applyColorMap(tmp, out, cv::COLORMAP_RAINBOW);
  }
...
/*    else if(param == "usbcamkinectlidar")
	...
	else if(param == "test")
    {
	  ...
	  useExact = false;
      ns = "";
      topicColor = "usb_cam/image_small";
      topicDepth = "kinect2/sd/image_depth_rect";
